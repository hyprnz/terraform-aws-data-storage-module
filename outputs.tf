...
output "rds_db_url" {
  ...
  value = element(concat(aws_db_instance.this.*.username, [""]), 0) == "" ? "" : format(
    "%s://%s:%s@%s/%s",
    var.rds_engine,
    element(concat(aws_db_instance.this.*.username, [""]), 0),
    var.rds_password,
    element(concat(aws_db_instance.this.*.endpoint, [""]), 0),
    element(concat(aws_db_instance.this.*.name, [""]), 0),
  )
}
